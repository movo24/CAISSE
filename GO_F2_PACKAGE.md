...
> Prérequis livrés : [F0+F1](PRODUCTS_FISCAL_STOCK_SYNTHESIS.md) sur `feat/stock-journal-nf525`.
...
- **Décision de signe à trancher (dans le GO)** : le journal veut `quantity` positif + direction
  par type, mais un ajustement peut être **+ ou −**. Proposition : autoriser `quantity` **signé**
  pour `inventory_adjust` (= `delta`), exception documentée à la règle « toujours positif »
  (l'alternative — deux types `adjust_in`/`adjust_out` — est plus lourde). Recommandation : delta signé.

### Tests (gated PG) / Rollback / Risques
- Tests : ajustement absolu et delta → un mouvement `inventory_adjust` avec le bon `delta` signé ; flag OFF → aucun mouvement ; audit inchangé.
- Rollback : flag OFF (aucun mouvement) ; additif, pas de migration.
- Risque : convention de signe — verrouiller au GO (delta signé) avant d'écrire.
