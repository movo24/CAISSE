...
## PAQUET 351 — Cycle X (2026-07-02) : fond de caisse + comptage persisté (POS-016/017/017b COMPLETS)
- Migration **1728-AddPosSessionCashFields** (3 colonnes nullables int sur `pos_sessions` : fond d'ouverture, compté, écart figé) — additive/réversible, dry-run pg-mem **2 tests**, file GATE 2 = 1725→**1728** (`run-gate2.sh` + MIGRATION_RUNBOOK + CLAUDE.md mis à jour, rollback ×4).
...
## PAQUETS 359→360 (2026-07-03) : incident P356 corrigé + situation multi-clones actée — ⚠️ COMMIT EN ATTENTE
- **P359 (incident auto-infligé, détecté indépendamment par les 2 agents)** : en rédigeant le plan de purge (P356, commit 2c60e58), l'ANCIENNE clé Google (déjà compromise depuis f2ad1b5 — PAS une nouvelle valeur) a été recollée en clair dans `SECRETS_REVOCATION_PLAN.md:54`, fichier tracké. Le garde `docs-no-secrets` l'a détectée (il fait son travail). Corrigé dans l'arbre de travail : ligne remplacée par une extraction `git show f2ad1b5` ; re-grep = 0 occurrence des 2 anciennes valeurs dans tout l'arbre (vérifié aux outils fichiers). Périmètre inchangé : aucun secret NOUVEAU, même valeur morte-à-révoquer, purge éventuelle déjà couverte par f2ad1b5.
- **P360 (constat multi-clones)** : scan complet de `~/CAISSE` — les seules lignes `PRIM_API_KEY=`/`GOOGLE_MAPS_API_KEY=` actives sont les placeholders de `.env.example`. Les 2 vraies clés posées par Omar via Claude Code vivent dans une AUTRE copie du projet (chemin à identifier : `git rev-parse --show-toplevel` côté Claude Code). CE clone tourne en no-key (fail-safe 4/4) — non bloquant. Voir TD-TWO-CLONES.
- **⚠️ ÉTAT D'EXÉCUTION** : la VM d'exécution du sandbox est HS (disque plein côté infrastructure, pas la machine d'Omar) depuis le milieu du P359 → git/jest indisponibles. Le fix P359 et cette entrée de log sont SUR DISQUE mais NON COMMITÉS, et `docs-no-secrets` n'a pas pu être re-exécuté. À la reprise (workspace revenu OU via Claude Code) : ① `git add SECRETS_REVOCATION_PLAN.md EXECUTION_LOG.md TECHNICAL_DEBT.md && git commit` ; ② re-run `docs-no-secrets` (attendu : vert) ; ③ régénérer le bundle.

## PAQUET 361 (2026-07-03) : POS-110 view-model + POS-036 réimpression — parts logicielles prouvées
- POS-110 : view-model pur `alerts-view.ts` (mobile) **8/8** — normalisation défensive (payload null/malformé → jamais de crash offline, overall recalculé selon la règle backend), badge FR 3 états, tri anomalies gravité→récence→id (déterministe, non mutant), sections ordonnées par gravité (vides omises). Reste : le composant écran (fetch + rendu, zéro logique).
- POS-036 : `reprint.ts` (pos-desktop) **6/6** — duplicata marqué tête+pied, montants COPIÉS de l'historique (jamais recalculés), date de la vente originale, compteur n°X, journal immuable {at,by}, et preuve bout-en-bout : la trame ESC/POS du duplicata passe par les builders prouvés (reset→DUPLICATA→coupe). Reste : l'impression BLE physique.
- mobile 4 fichiers/21 tests PASS ; pos-desktop tsc RC 0 + 14 fichiers/83 tests PASS. Note : 2 erreurs tsc pré-existantes sur mobile (`ImportMeta.env`, fichiers non touchés — typage vite hors build), build vite = référence.

## PAQUET 362 (2026-07-03) : POS-110 COMPLET côté logiciel — écran AlertsPage mobile livré
- `AlertsPage.tsx` : rendu pur sur le view-model prouvé (8/8) — badge global, sections gravité, anomalies triées, listes stock critique/bas ; erreurs affichées avec Réessayer (S5) ; header « Lecture seule — aucune action possible ici » ; refresh manuel.
- Câblage : `cockpitApi.alerts()` (`GET /mobile/v1/alerts`, JWT employé), route `/alerts`, tuile Home « Supervision » visible **manager/admin uniquement** (`canModifyStock` — même garde que l'endpoint). Incident bénin : import lucide cassé par le script d'insertion, corrigé immédiatement (build rouge→vert dans le même paquet).
- Preuves : mobile build vite RC 0 · vitest 4 fichiers/21 tests PASS · tsc (hors 2 erreurs ImportMeta pré-existantes) propre. Reste hors logiciel : test sur smartphone réel.

## PAQUET 363 (2026-07-03) : dossier d'exécution Omar + POS-019 clos — côté logiciel ÉPUISÉ
- **`DOSSIER_EXECUTION_OMAR.md`** livré : §1 Gate 2 (commande unique, prérequis, table d'erreurs→actions, quoi me coller, plan de reprise immédiat en 3 étapes) ; §2 e2e locaux POS-010→015 (Playwright DÉJÀ installé — smoke login→scan→espèces existant, séquence exacte 3 commandes, critères VERT/ROUGE par bloc, 4 scénarios S2-S5 à écrire dès confirmation) ; §3 runbooks matériel (TPE WisePad mode test + preuve anti-double-charge réelle + différé, imprimante BLE + duplicata + tiroir, caméra + anti-rebond réel, smartphone POS-110) ; §4 tableau FINAL des 8 gates avec propriétaire et ordre recommandé (G2→G1→G3→G4→G5).
