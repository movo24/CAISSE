...
## D16 — Couplage « op committée ⟺ audit committée » NON garanti  (M402 review)
**Status:** OPEN · **P1 (NF525 si la chaîne d'audit est dans le périmètre — cf D17).** Confirmé par le code : `AuditService.log` n'accepte jamais de `queryRunner` → il écrit dans une **transaction séparée**, et tous les appels en vente sont **post-commit** dans `try/catch → logger.warn`. Donc une op peut committer alors que son entrée d'audit échoue (best-effort). **M402 n'a PAS modifié ce couplage** (il n'a jamais existé) ; il a changé le mode d'échec sous contention de tête : *fork silencieux* → *retry puis, si épuisé, drop loggué en WARN*. **Décision d'architecture (owner)** : (a) audit **dans la txn métier** = fail-closed (l'op échoue avec son audit ; l'audit devient point de contention sur la vente) vs (b) garder out-of-band best-effort + au minimum **escalader le drop en ALERTE** (pas WARN) + compteur. **Ferme :** trancher (a)/(b) ; si (b), AlertService sur épuisement de retry + monitoring.
