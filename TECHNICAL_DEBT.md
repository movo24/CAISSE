...
## D20 — Mouvements stock-locations non audités (couverture)  — P2, additif
**Status:** OPEN · **P2.** `stock-locations.service` injecte `AuditService` mais ses 4 méthodes transactionnelles (`receiveFromSupplier`, `transfer`, `recordLoss`, `dispatch`) **n'appellent jamais `audit.log`** → réception/transfert/perte/dispatch de stock ne laissent **aucune trace d'audit** (dead injection). Ce n'est pas un phantom (rien n'est écrit) ni un trou de sécu, mais une **lacune de couverture d'audit** sur des mouvements de stock réels. **Ferme :** ajouter un audit **post-commit best-effort** (modèle ratifié D16/D17 : out-of-band) après chaque méthode, avec ancien/nouveau + acteur ; tests. Additif, à faire en continuité.
...
## D21 — Branche succès TPE non câblée  (M601) — diagnostic 2026-06-24
**Status:** OPEN · **P1 (paiement carte).** Vérifié en lecture sur `main` (`8cbf5d8`) : `handleTpeResponse('success'|'refused'|'timeout')` (`pos-desktop usePayment.ts:343`, dupliqué `POSPage.tsx:580`) n'est **jamais appelé avec `'success'` ni `'refused'`** — seul `'timeout'` l'est, depuis le timer de countdown (`usePayment.ts:374`, `POSPage.tsx:570`). Aucun listener `peripheralBridge`/terminal n'émet le résultat, aucun bouton de confirmation manuel. ⇒ une CB réellement approuvée au TPE 4G **time out** dans le POS ; la branche succès (`usePayment.ts:347-362` → `finalizePayment`/`commitPartialPayment` ; `POSPage.tsx:591-625`) est **code mort**.
**Ferme (Tier-2 paiement — GO requis)** : câbler un résultat terminal réel → `handleTpeResponse('success'|'refused')`, **ou** ajouter des boutons "Paiement accepté / Refusé" à l'overlay TPE (réaliste pour un TPE 4G autonome ; cf roadmap "bouton confirm"). Doit respecter decision-6 (pas de "payé" sans capture réelle : confirm manuel = attestation caissier). Doit couvrir `usePayment` **et** `POSPage` (cf D22).

**Nouvelle surface fiscale — point d'attestation humaine (à acter).** Avec un TPE 4G autonome il n'y a **aucune capture électronique** : le "payé" dépend désormais d'une **assertion caissier** (bouton "Paiement accepté"). Un faux `success` (misclick, caissier non formé) écrit une **vente payée dans la chaîne fiscale sans encaissement réel**. Ce n'est pas un bug — c'est inhérent au TPE autonome — mais c'est un point d'attestation humaine qui n'existait pas. **Garde-fous en place** : `refused`/`timeout`/no-context ne finalisent **jamais** (decideTpeOutcome, decision-6). **Mitigation** = formation caissier. **Cible** = lecteur intégré (capture réelle → `handleTpeResponse('success')` sur événement, pas sur clic). À tester runtime avant merge (cf ci-dessous).

**Fix prêt, NON CONFIRMÉ runtime (branche `fix/pos-tpe-card-success-2026-06`, `25d44b7`, non mergé).** `decideTpeOutcome` testé (vitest 6/6) ; câblage **bouton→`handleTpeResponse`→finalize prouvé par tsc+lecture seulement** (un code mort compile aussi → la méthode de vérif est celle qui n'a pas attrapé le bug). **Manque** : QA manuelle clic sur les deux overlays (POSPage + iPad) OU infra test composant (`@testing-library/react` + `jsdom` absents ; pas d'`npm install` en worktree). Pas de merge tant que (a) runtime confirmé, (b) CI réel vert (pas 404).

## D22 — Logique de paiement dupliquée `POSPage.tsx` ⟂ `usePayment.ts`  — diagnostic 2026-06-24
**Status:** OPEN · **P2 (maintenabilité).** `handleTpeResponse`, `commitPartialPayment`, finalize existent **dans les deux** fichiers (`usePayment.ts` ET `POSPage.tsx`). Les builders de payload sont partagés (`salePayload.ts`) donc le **payload wire ne dérive pas** (M603 fermé), mais l'**orchestration** est dupliquée ⇒ un fix paiement (ex. M601) doit être appliqué aux deux, ou les deux consolidés d'abord. **Ferme :** consolider la logique paiement dans un seul hook (`usePayment`) consommé par `POSPage` ; tests. Additif/réversible, mais touche le flux paiement ⇒ revue.
