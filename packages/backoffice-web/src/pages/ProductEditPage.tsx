...
const inputCls = 'w-full px-3.5 py-2.5 rounded-xl border border-gray-200 text-sm bg-white focus:outline-none focus:ring-2 focus:ring-[#E5117A]/20 focus:border-[#E5117A]/50';
...
export function ProductEditPage() {
  ...
  const [form, setForm] = useState({
    ...
    stock: '0', alertThreshold: '10', criticalThreshold: '5', imageUrl: '', isActive: true,
  });
  ...
  const load = useCallback(async () => {
    try {
      const [b, s] = await Promise.all([productsApi.listBrands(), productsApi.listSuppliers()]);
    ...
    return (
      <div className="p-6 lg:p-8 max-w-2xl mx-auto animate-fade-in">
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-soft p-8 text-center space-y-5">
          <div className="w-14 h-14 mx-auto rounded-2xl bg-[#E5117A]/10 flex items-center justify-center"><ScanBarcode size={26} className="text-[#E5117A]" /></div>
          <div>
          ...
          {gateExisting && (
            <div className="rounded-xl border border-amber-200 bg-amber-50 p-4 text-left">
              ...
              <button onClick={() => navigate(`/products/${gateExisting.id}/edit`)} className="mt-3 inline-flex items-center gap-1.5 px-3 py-2 rounded-lg bg-[#E5117A] text-white text-xs font-semibold"><Pencil size={13} /> Éditer le produit existant</button>
            </div>
          )}
          <div className="flex gap-2 justify-center">
            <button onClick={checkEan} disabled={gateBusy || !gateEan.trim()} className="px-5 py-2.5 rounded-xl bg-[#E5117A] text-white text-sm font-semibold disabled:opacity-40 flex items-center gap-2">
    ...
    <div className="p-6 lg:p-8 space-y-5 animate-fade-in max-w-[1200px] mx-auto">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        ...
        <div className="flex items-center gap-2">
          ...
          <button onClick={save} disabled={saving} className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-[#E5117A] text-white text-sm font-semibold disabled:opacity-40">
      ...
      <div className="flex gap-1 overflow-x-auto border-b border-gray-200 pb-px">
        {TABS.map(({ key, label, icon: Icon }) => (
          <button key={key} onClick={() => setTab(key)}
            className={`flex items-center gap-1.5 px-3.5 py-2.5 text-xs font-semibold rounded-t-lg whitespace-nowrap transition-colors border-b-2 ${tab === key ? 'text-[#E5117A] border-[#E5117A] bg-[#E5117A]/[0.04]' : 'text-gray-400 border-transparent hover:text-gray-600'}`}>
            <Icon size={14} /> {label}
          </button>
        ))}
      </div>
        ...
        {tab === 'general' && (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            ...
            <div className="space-y-5">
              <Field label="Catégorie / famille"><input className={inputCls} value={form.categoryId} onChange={(e) => set('categoryId', e.target.value)} placeholder="ex : boissons" /></Field>
            ...
            <Field label="Marque">
              <div className="flex gap-2">
                ...
                <button onClick={createBrand} className="px-3 rounded-xl border border-gray-200 text-gray-500 hover:text-[#E5117A]" title="Nouvelle marque"><Plus size={15} /></button>
              </div>
            </Field>
            ...
            <Field label="Statut">
              <label className="flex items-center gap-2 text-sm text-gray-600 mt-2">
                <input type="checkbox" checked={form.isActive} onChange={(e) => set('isActive', e.target.checked)} className="accent-[#E5117A]" /> Produit actif (vendable en caisse)
              </label>
            </Field>
          ...
          <div className="space-y-6">
            <Field label="Fournisseur principal">
              <div className="flex gap-2">
                ...
                <button onClick={createSupplier} className="px-3 rounded-xl border border-gray-200 text-gray-500 hover:text-[#E5117A]" title="Nouveau fournisseur"><Plus size={15} /></button>
              </div>
            </Field>
            {supplier && (
            ...
            {!isEdit ? <p className="text-sm text-gray-400">Enregistrez d'abord la fiche pour composer un pack.</p> : (
              <>
                <div className="flex gap-2 items-end flex-wrap">
                  ...
                  <button onClick={addComponent} className="px-4 py-2.5 rounded-xl bg-[#E5117A] text-white text-sm font-semibold flex items-center gap-1.5"><Plus size={14} /> Ajouter</button>
                </div>
            ...
            {!isEdit ? <p className="text-sm text-gray-400">Enregistrez d'abord la fiche pour créer des variantes.</p> : original?.parentProductId ? (
              ...
              <>
                <div className="grid grid-cols-1 md:grid-cols-4 gap-3 items-end">
                  ...
                  <button onClick={addVariant} className="px-4 py-2.5 rounded-xl bg-[#E5117A] text-white text-sm font-semibold flex items-center gap-1.5"><Plus size={14} /> Créer</button>
                </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5 items-start">
              <Field label="Photo principale (URL ou fichier)" hint="Un fichier choisi est converti en data-URL (stockage existant). Vider = retirer l'image.">
                ...
                <div className="flex gap-2 mt-2">
                  <label className="px-3 py-2 rounded-lg border border-gray-200 text-xs font-semibold text-gray-600 cursor-pointer hover:border-[#E5117A]">
                    Choisir un fichier…
            ...
            {!isEdit ? <p className="text-sm text-gray-400">Enregistrez d'abord la fiche.</p> : (
              <>
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-3 items-end">
                  ...
                  <button onClick={savePromo} className="px-4 py-2.5 rounded-xl bg-[#E5117A] text-white text-sm font-semibold">Programmer</button>
                </div>
