...
export function ProductCard({ product, onClose, onStockUpdated }: ProductCardProps) {
  ...
  const handleAdjust = async () => {
    ...
    try {
      ...
      const newQty = product.stockQuantity + adjustQty;
      setSuccess(true);
      ...
      setError(err.response?.data?.message || 'Erreur ajustement stock');
    } finally {
      setAdjusting(false);
    }
  };
        ...
        <div className="px-5 pb-6">
          ...
          <div className="flex gap-4 mb-5">
            ...
            <div className="flex-1 min-w-0">
              <h2 className="text-base font-bold text-mobile-text leading-tight">
                {product.name}
              </h2>
              {product.ean && (
                <p className="text-xs text-mobile-muted font-mono mt-1">{product.ean}</p>
              )}
              {product.categoryId && (
          ...
          <div className="flex gap-3 mb-5">
            <div className="flex-1 px-4 py-3 rounded-xl bg-gray-50">
              ...
              <p className="text-lg font-bold text-mobile-text mt-0.5">
                {(product.priceMinorUnits / 100).toFixed(2)} <span className="text-sm">EUR</span>
              </p>
            </div>
          ...
          {canModifyStock() && (
            <div className="pt-4 border-t border-gray-100">
              <p className="text-xs font-semibold text-mobile-text mb-3">
                Ajuster le stock
              </p>
              ...
              <button
                ...
                className={`w-full py-3.5 rounded-2xl font-bold text-sm transition-all disabled:opacity-30 ${
                  adjustQty > 0
