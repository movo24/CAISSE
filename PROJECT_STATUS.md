...
| 10 | Imprimante ticket | 🔄 | ESC/POS Bluetooth **réel, monté iPad** (`IPadPOSLayout.tsx:99`) — chemin V1 prioritaire. **Impression honnête (PR #27)** : auto-print sans fallback boîte de dialogue (`allowBrowserFallback:false`) — un échec thermique remonte `print_failed`, jamais un faux succès ; statut par vente (`printed`/`print_failed`/`no_printer`) affiché sur l'écran de confirmation des DEUX chemins (« Aucune imprimante connectée — ticket NON imprimé »). Desktop : toujours pas d'impression réelle (electronAPI mort) mais **le poste le dit clairement**. Réimpression manuelle = fallback navigateur conservé (action explicite) |
...
**PR #24 — Idempotency-Key vente online** ✅ · **PR #25 — Carte réelle WisePad 3** ✅ · **PR #26 — Chemin desktop inline sécurisé** ✅ · **PR #27 — Impression honnête** ✅ (statut par vente, pas de faux succès, plateformes muettes le disent). Suite de la roadmap terrain (ordre imposé owner) :
